...
function run() {
  ...
  console.log("[START] Running start functions...");
  ...
  schedule.scheduleJob("0 0 * * *", function(fireDate){
    console.log(`[START] Started giving bullets back (${fireDate}).`);
...
async function give_all_bullets_back() {
  ...
  console.log(`[START] Last time giving bullets back was done was on ${previous}.`);
  let month = d.getMonth()+1;
  if(month<10) month = '0' + month;
  let day = d.getDate();
  if(day<10) day = '0' + day;
  const today = `${d.getFullYear()}-${month}-${day}`;
  console.log(`[START] Today is ${today}.`);
  ...
  if(previous == today){
    ...
    console.log("[START] Gave bullets back!");
  }
